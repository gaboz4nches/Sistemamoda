...
<div class="col-md-8 offset-2">
  ...
  {{-- @include('layouts.validation_errors') --}}
  	<form action="{{ url('affiliated_companies/'.$affis->id) }}" method="post" enctype="multipart/form-data">
  		@method('put')
		  {!! csrf_field() !!}
      ...
      <div class="form-group">
        <input type="hidden" name="id" value="{{ $affis->id }}">
        <input type="number" name="contacto" class="form-control" placeholder="Numero de contacto" value="{{ $affis->contacto }}">
      </div>
  		<div class="form-group">
        <input type="text" name="direccion" class="form-control" placeholder="Direccion" value="{{ $affis->direccion }}">
  		</div>
      <div class="form-group">
        <input type="text" name="web" class="form-control" placeholder="Sitio Web" value="{{ $affis->web }}" autocomplete="off">
      </div>
  		<div class="form-group">
